...
| **Updates / caching** | SW cache-bust + version gate + cache-first navigation | No SW; updates via Web Store (or reload-unpacked). "Check for Updates" fetches the live `version.js` (CORS) and names the Web Store when newer (`utils/updateCheck.js`) | Different distribution model |
