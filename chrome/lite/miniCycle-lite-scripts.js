...
var editionLabel = 'Chrome Edition v1.0';

// ✅ ADD version display function
function showVersionInfo() {
  console.log('📱 miniCycle Lite ' + editionLabel + ' (engine v' + currentVersion + ')');
  ...
  if (footerLinks) {
    ...
    if (!versionElement) {
      ...
      li.textContent = editionLabel;
      footerLinks.appendChild(li);
    } else {
      versionElement.textContent = editionLabel;
    }
  }
}
...
function addTask(taskText, completed, shouldSave, dueDate, highPriority, isLoading) {
  ...
  // ✅ Priority indicator (red exclamation circle SVG)
  var priorityIndicator = document.createElement("span");
  priorityIndicator.className = "priority-indicator";
  priorityIndicator.innerHTML = '<svg viewBox="0 0 24 24" width="18" height="18" fill="currentColor" aria-hidden="true"><path d="M12 2a10 10 0 1 0 10 10A10 10 0 0 0 12 2m1 15h-2v-2h2v2m0-4h-2V7h2v6z"/></svg>';
...
// Focus mode — toggle a class on <body> that hides chrome and gives the
// task list more room. CSS handles all visibility (including swapping the
...
// Open the full miniCycle web app in a new tab (extension popup version)
function handleTryFullVersion() {
  ...
  window.open('https://minicycleapp.com', '_blank');
}
...
function handleFeedbackError() {
  ...
  // Hide the form so the error block has its own space (mirrors success path)
  if (feedbackForm) feedbackForm.style.display = 'none';

  if (thankYouMessage) {
    thankYouMessage.className = 'feedback-error-block';
    thankYouMessage.style.display = 'block';
    thankYouMessage.style.color = ''; // clear any prior inline color
    thankYouMessage.innerHTML =
      '<div class="feedback-error-icon">⚠️</div>' +
      '<div class="feedback-error-title">Failed to send feedback</div>' +
      '<div class="feedback-error-body">Please try again later or contact support.</div>';
  }
...
function showEmptyState() {
  ...
  emptyState.innerHTML = [
    '<div class="empty-state-content">',
      ...
      '<p class="empty-state-subtitle">The lightweight version, right in your toolbar</p>',
      ...
      '<div class="lite-version-info">',
        '<p class="lite-info-text">',
          ...
          'Looking for more features? The full version is available at ',
          '<a href="https://minicycleapp.com" target="_blank" rel="noopener">minicycleapp.com</a>.',
        '</p>',
      '</div>',
      ...
      '<div class="quick-tips">',
        '<h4>💡 Quick Tips:</h4>',
        '<ul class="tips-list">',
          '<li>Tap + Add Tasks to get started</li>',
          '<li>Tap the three dots (⋯) on a task for more options</li>',
          '<li>Use the Menu button to access settings and send feedback</li>',
