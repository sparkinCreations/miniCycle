...
> For the detailed reference, see `web/docs/working-on-code/CLAUDE.md`.
...
- **Wiring a shared function across modules** → `web/docs/working-on-code/MAKING_CODE_CHANGES.md` (the 4-step DI pipeline; missing any layer = silent `undefined`)
- **Adding new constants / IDs / classes / selectors** → `web/docs/working-on-code/CONSTANTS_SYSTEM_GUIDE.md` (when to use `DOM_IDS` vs `DOM_CLASSES` vs `DOM_SELECTORS`, naming conventions)
- **Adding a new module or DI dependency** → `web/docs/working-on-code/DI_PATTERNS.md` and `web/docs/architecture/MODULE_LOADER_GUIDE.md`
...
npm run validate:keys     # no object literal may define the same key twice — the later one silently WINS and the earlier is dead code. Not a syntax error, not a lint error, and validate:labels passes because a shadowed key still RESOLVES, just to the wrong string. esbuild has always warned, buried in a build log ending "dist/ ready". Measured Sep 2026: notify.reminderEnabled was defined twice, so the reminders notification dropped the schedule it had just computed (CI; gated at 0)
npm run validate:chains   # a required() dep may never be read through `?.` — that branch only fires when wiring is broken, and then it silently drops the feature. Also catches local aliases (`const AppState = deps.AppState || _deps.AppState` then `AppState?.`), which the hot path used everywhere and the gate missed until Sep 2026 (CI; gated at 0; catch blocks exempt)
...
npm run validate:provides # a manifest `provides` name must actually exist on the module, and no two manifests may claim the same name — registerProvides SKIPS a name it cannot find and a direct instance call resolves to undefined, both silently; that is how the v2.347 statsPanel split broke three-panel swipe until v2.387, found on a phone, not by a test (CI; unsupplied ratcheted by name, duplicates gated at 0)
npm run validate:reset  # every STORAGE_KEYS entry must be swept, listed, or deliberately preserved by the factory reset — the sweep matches only names containing "minicycle"/"taskcycle", and pluginIntegrationGuide tells plugin authors to name keys like `timeTrackerData`, so an unswept key SURVIVES a reset the user was told deleted everything, with no error and a success notification (CI; gated at 0)
