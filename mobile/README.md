...
Same shape as Android: the **byte-identical** web app inside a Capacitor WKWebView, bundled for
offline. The web payload (`mobile/ios/www/`) is generated by `web/scripts/build-ios-www.cjs` —
both platform scripts are thin shims over the shared engine `build-capacitor-www.cjs`, so the
transform can never drift between Android and iOS. The Xcode project uses **Swift Package
...
npm install        # first time only
npm run sync       # generate www/ from web/ + cap sync
npm run open       # open in Xcode to build & run (requires full Xcode)
```
