...
export async function runNotificationDialogHostTests(resultsDiv) {
    ...
    await test('showModal() reparents container into dialog', async () => {
    ...
    await test('close() moves container back to body', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
            dialog.showModal();
            await wait(0);
            dialog.close();  // close event + reparent are async — must await
            await wait(0);
    ...
    await test('container appended as LAST child (not :first-child)', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
            dialog.showModal();
    ...
    await test('second modal becomes new host for container', async () => {
        const container = ensureNotificationContainer();
    ...
    await test('closing inner modal returns container to outer modal', async () => {
        const container = ensureNotificationContainer();
        const first = makeDialog();
        const second = makeDialog();
        try {
    ...
    await test('closing ALL nested modals returns container to body', async () => {
        const container = ensureNotificationContainer();
        const first = makeDialog();
        const second = makeDialog();
        try {
    ...
    await test('non-modal dialog.show() is ignored', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
    ...
    await test('dialog.close() followed by dialog.remove() — container survives', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
            dialog.showModal();
    ...
    await test('dialog.open=false (attribute remove) also triggers reparent', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
            dialog.showModal();
    ...
    await test('escape key close also returns container', async () => {
        const container = ensureNotificationContainer();
        const dialog = makeDialog();
        try {
            dialog.showModal();
    ...
    await test('destroy() sets initialized=false', async () => {
    ...
    await test('destroy() clears internal stack', async () => {
        const throwaway = new NotificationDialogHost({
            getBody: () => document.body,
            waitForCore: () => Promise.resolve()
        });
    ...
    // ===== SUMMARY =====
    // Standard "Results:" h3 so the automated runner detects pass/total (run-browser-tests.cjs).
    const percentage = total.count ? Math.round((passed.count / total.count) * 100) : 0;
    resultsDiv.innerHTML += `
        <h4 class="test-section">📊 Summary</h4>
        <h3>Results: ${passed.count}/${total.count} tests passed (${percentage}%)</h3>
    `;
