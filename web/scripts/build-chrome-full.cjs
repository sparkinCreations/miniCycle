...
function writeManifestAndBackground(version) {
  const manifest = {
    ...
    // "storage" has NO install warning. It lets the background SW remember the
    // launcher tab's id (in chrome.storage.session) so the toolbar click focuses
    // the already-open tab instead of opening duplicates. We deliberately avoid
    // the "tabs" permission — it would add a scary "Read your browsing history"
    // warning just to let tabs.query() match our own page URL.
    permissions: ['storage'],
    icons: {
      16: 'icons/icon-16.png',
      32: 'icons/icon-32.png',
