...
- **statsPanel owns the view switch:** `showTaskView()`/`showStatsPanel()` (statsPanel.js:774/811) do class toggles + `inert` + slide-arrow indicators + `_syncGestureManager` + `announceViewChange` + `updateNavDots`. `updateNavDots`/`handleDotClick` are hard-coded to indexes 0/1. `handleNavPillClick` toggles. Both `show*` functions are **DI-provided public APIs with external consumers — their signatures must not change.**
  - **Update (v2.309, July 20 2026):** dot CLICKS now navigate directly to the dot's own panel via its `aria-controls` + `carousel.goTo()` (in focus view the dots are labeled **Task | Routine | Stats** targets — toggling cycled views instead of honoring the click). The container pill click keeps the legacy toggle, and `goTo()`'s isEnabled gates still protect the Task panel outside focus view.
...
**Post-ship fix (July 11, 2026 — returning-user empty card):** users with PERSISTED focus mode (`settings.focusModeActive`) got an empty skeleton card: `focusMode.init()` restores via `activate(true)` MID-Phase-2, but `deps.core.ensureModuleLoaded` was wired only AFTER `loadAllModules()` returned — the depMappings closure silently no-op'd (truthy-closure trap) and the panel module never loaded. New users were unaffected (onboarding activates post-boot). Fix: featureBoot wires `ensureModuleLoaded` BEFORE `loadAllModules` (safe — boot context is captured as loadAllModules' first step), and `focusMode.activate()` now warns loudly if the loader resolves undefined. Verified with a persisted-focus-mode reload repro (6/6): module loads during boot-restore, card fully rendered.
