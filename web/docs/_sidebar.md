<!-- _sidebar.md -->

* **Start Here** _(read 1 → 7 in order)_
  * [Home](/)
  * [1. What is miniCycle?](start-here/WHAT_IS_MINICYCLE.md)
  * [2. How miniCycle Works](start-here/HOW_MINICYCLE_WORKS.md)
  * [3. Your First Contribution](start-here/FIRST_CONTRIBUTION.md)
  * [4. Getting Started](start-here/GETTING_STARTED.md)
  * [5. Framework Equivalents](start-here/FRAMEWORK_EQUIVALENTS.md)
  * [6. Folder Structure](start-here/FOLDER_STRUCTURE.md)
  * [7. Dev Server](start-here/DEV_SERVER.md)

* **User Guides**
  * [User Guide](user-guides/USER_GUIDE.md)
  * [Quick Reference](user-guides/QUICK_REFERENCE.md)
  * [FAQ](user-guides/FAQ.md)

* **Project Info**
  * [Roadmap](project-info/ROADMAP.md)
  * [Contributing](project-info/CONTRIBUTING.md)
  * [Privacy Policy](project-info/PRIVACY.md)
  * [Browser Compatibility](project-info/BROWSER_COMPATIBILITY.md)
  * [Accessibility](project-info/ACCESSIBILITY.md)
  * [App Reviews](project-info/APP_REVIEWS.md)
  * [Copyright & Authorship](project-info/COPYRIGHT_AUTHORSHIP_STATEMENT.md)

* **Working on Code** _(how-to — followed while working)_
  * [Making Code Changes](working-on-code/MAKING_CODE_CHANGES.md)
  * [How-To Cookbook](working-on-code/HOW_TO_ADD_COOKBOOK.md)
  * [Coding Standards](working-on-code/CODING_STANDARDS.md)
  * [DI Patterns](working-on-code/DI_PATTERNS.md)
  * [Development Workflow](working-on-code/DEVELOPMENT_WORKFLOW.md)
  * [Event Listener Guide](working-on-code/EVENT_LISTENER_GUIDE.md)
  * [Async UI Patterns](working-on-code/ASYNC_UI_PATTERNS.md)
  * [Messaging Surfaces](working-on-code/MESSAGING_SURFACES.md)
  * [Constants System Guide](working-on-code/CONSTANTS_SYSTEM_GUIDE.md)
  * [Security Guide](working-on-code/SECURITY_GUIDE.md)
  * [Error Recovery & Salvage](working-on-code/ERROR_RECOVERY.md)
  * [Testing Guide](working-on-code/TESTING_GUIDE.md)
  * [Validation Gates](working-on-code/VALIDATION_GATES.md)
  * [Debug Mode](working-on-code/DEBUG_MODE.md)
  * [Hidden Codebase Insights](working-on-code/HIDDEN_CODEBASE_INSIGHTS.md)
  * [AI-Assisted Development](working-on-code/AI_ASSISTED_DEVELOPMENT.md)
  * [Development Assistant Notes](working-on-code/DEVELOPMENT_ASSISTANT_NOTES.md)
  * [Claude AI Guide](working-on-code/CLAUDE.md)

* **Architecture** _(why it's built this way)_
  * [Architecture Decision Records](ARCHITECTURE_DECISIONS.md)
  * [Architecture Overview](architecture/ARCHITECTURE_OVERVIEW.md)
  * [Module System Guide](architecture/MODULE_SYSTEM_GUIDE.md)
  * [Module Loader Guide](architecture/MODULE_LOADER_GUIDE.md)
  * [AppInit System](architecture/APPINIT_SYSTEM.md)
  * [AppInit Explained](architecture/APPINIT_EXPLAINED.md)
  * [Dependency Map](architecture/DEPENDENCY_MAP.md)
  * [Event Flow Patterns](architecture/EVENT_FLOW_PATTERNS.md)
  * [Undo/Redo Architecture](architecture/UNDO_REDO_ARCHITECTURE.md)
  * [History System](architecture/HISTORY_SYSTEM.md)
  * [Quick Actions & Usage Tracking](architecture/QUICK_ACTIONS_ARCHITECTURE.md)
  * [Drag & Drop Architecture](architecture/DRAG_DROP_ARCHITECTURE.md)
  * [Drag & Drop Long Press](architecture/DRAG_DROP_LONG_PRESS.md)
  * [Mode Manager Architecture](architecture/MODE_MANAGER_ARCHITECTURE.md)
  * [Routine Switcher Architecture](architecture/ROUTINE_SWITCHER_ARCHITECTURE.md)
  * [Recurring Watch Function](architecture/RECURRING_WATCH_FUNCTION.md)
  * [Theme Architecture](architecture/THEME_ARCHITECTURE.md)
  * [Label System Architecture](architecture/LABEL_SYSTEM_ARCHITECTURE.md)
  * [CSS Architecture Guide](architecture/CSS_ARCHITECTURE_GUIDE.md)
  * [Background Pattern](architecture/BACKGROUND_PATTERN.md)
  * [Lite Version](architecture/LITE_VERSION.md)

* **Reference** _(look it up)_
  * [Project Stats](PROJECT_STATS.md)
  * [API Reference](reference/API_REFERENCE.md)
  * [Feature List](reference/FEATURE_LIST.md)
  * [Schema 2.5](reference/SCHEMA_2_5.md)
  * [MCYC File Format](reference/MCYC_FILE_FORMAT.md)
  * [Data Schema Guide](reference/DATA_SCHEMA_GUIDE.md)
  * [Label Registry Reference](reference/LABEL_REGISTRY_REFERENCE.md)
  * [Recurring System Reference](reference/RECURRING_SYSTEM_REFERENCE.md)
  * [TaskDOM DI Guide](reference/TASKDOM_DI_GUIDE.md)
  * [Review Patterns](reference/REVIEW_PATTERNS.md)
  * [Drift Audit Checklist](DRIFT_AUDIT_CHECKLIST.md)
  * [Documentation Index](INDEX.md)
  * [Developer Documentation](DEVELOPER_DOCUMENTATION.md)

* **Features**
  * [Stats Panel](features/STATS_PANEL.md)
  * [Storage Management](features/STORAGE_MANAGEMENT.md)
  * [Completed Tasks Dropdown](features/COMPLETED_TASKS_DROPDOWN.md)
  * [Task Options Customizer](features/TASK_OPTIONS_CUSTOMIZER.md)
  * [Auto-Uncheck Daily](features/AUTO_UNCHECK_DAILY.md)
  * [Focus Task View](features/FOCUS_TASK_VIEW.md)
  * [UX Ratings](features/UX_RATINGS.md)
  * [Vocabulary Theme System](features/VOCAB_THEME_SYSTEM.md)
  * [Sample Routines](features/SAMPLE_ROUTINES.md)
  * [SVG Icon System](features/SVG_ICON_SYSTEM.md)
  * [SVG Animation Overlay](features/SVG_ANIMATION_OVERLAY_GUIDE.md)
  * [First-Run Welcome Animation](features/FIRST_RUN_WELCOME_ANIMATION.md)
  * [First-Run Welcome Banner](features/FIRST_RUN_WELCOME_BANNER.md)
  * [Confirmation & Notification Modal](features/confirmation-and-notification-modal.md)
  * [Module Notifications Guide](features/miniCycle-module-notifications-guide.md)

* **Testing & Performance**
  * [Testing Architecture](testing/TESTING_ARCHITECTURE.md)
  * [Testing README](testing/TESTING_README.md)
  * [Testing Quick Reference](testing/TESTING_QUICK_REFERENCE.md)
  * [Testing Modal](testing/TESTING_MODAL.md)
  * [Performance Testing Guide](performance/PERFORMANCE_TESTING_GUIDE.md)
  * [Performance Setup](performance/PERFORMANCE_SETUP.md)
  * [Performance Summary](performance/PERFORMANCE_SUMMARY.md)
  * [Lighthouse Benchmarks](performance/LIGHTHOUSE_BENCHMARKS.md)

* **Security & Error Handling**
  * [📊 Complete Summary (Start Here)](archive/ERROR_HANDLING_AND_TESTING_SUMMARY.md)
  * [Error Handling Improvements](archive/ERROR_HANDLING_IMPROVEMENTS.md)
  * [Security Overview](security/SECURITY.md)
  * [CSP & .htaccess Guide](security/CSP_AND_HTACCESS_GUIDE.md)
  * [Error Handling Audit](security/audits/AUDIT_README.md)
  * [Audit Summary (Executive)](security/audits/AUDIT_SUMMARY.md)
  * [Error Handling Analysis](security/audits/ERROR_HANDLING_AUDIT.md)
  * [File-by-File Analysis](security/audits/FILE_ERROR_HANDLING_ANALYSIS.md)

* **Incidents** _(postmortems)_
  * [Service Worker Stale Cache](incidents/INCIDENT_service-worker-stale-cache.md)
  * [Stale Cache Fix #3](incidents/FIX_3_STALE_CACHE.md)
  * [Undo/Redo Rollback UI Refresh](incidents/BUG_undo-redo-rollback-ui-refresh.md)
  * [boot-sw Classic-Script ESM Break (v2.346)](incidents/INCIDENT_boot-sw-classic-script-esm.md)

* **Deployment & Updates**
  * [Deployment Guide](deployment/DEPLOYMENT.md)
  * [Build Process (esbuild pipeline)](deployment/BUILD_PROCESS.md)
  * [PWA Offline Architecture](deployment/PWA_OFFLINE_ARCHITECTURE.md)
  * [Service Worker Update Strategy](deployment/SERVICE_WORKER_UPDATE_STRATEGY.md)
  * [Update Version Guide](deployment/UPDATE-VERSION-GUIDE.md)
  * [CTA Click Tracking](deployment/CLICK_TRACKING.md)

* **Future Work** _(active plans only)_
  * [AppState Merge States](future-work/APPSTATE_MERGE_STATES.md)
  * [Architecture Review Findings](future-work/ARCHITECTURE%20REVIEW%20FINDINGS.md)
  * [Auto-Generated depMappings](future-work/AUTO_GENERATED_DEPMAPPINGS_PLAN.md)
  * [Boot Performance Roadmap](future-work/BOOT_PERF_ROADMAP.md)
  * [Code Consistency Audit](future-work/CODE_CONSISTENCY_AUDIT.md)
  * [Completed Dropdown Integration](future-work/COMPLETED_DROPDOWN_INTEGRATION_PLAN.md)
  * [Docs Reorganization Plan](future-work/DOCS_REORG_PLAN.md)
  * [Enforce Requires Rollout](future-work/ENFORCE_REQUIRES_ROLLOUT_PLAN.md)
  * [Error Handling — Phases 3–5](future-work/ERROR_HANDLING_PHASES_3_5.md)
  * [Feedback TODO (Jul 2026)](future-work/FEEDBACK_TODO_2026_07.md)
  * [highPriority null Default Fix](future-work/HIGH_PRIORITY_NULL_DEFAULT_FIX.md)
  * [i18n Language Packs](future-work/I18N_LANGUAGE_PACK_PLAN.md)
  * [Large Module Splits](future-work/LARGE_MODULE_SPLITS_PLAN.md)
  * [Modal Access Centralization](future-work/MODAL_ACCESS_CENTRALIZATION_PLAN.md)
  * [Module Deferral Audit](future-work/MODULE_DEFERRAL_AUDIT.md)
  * [Native Dialog Audit & Refactor](future-work/NATIVE_DIALOG_AUDIT_AND_REFACTOR_PLAN.md)
  * [Pretty-URL Cache-Control Fix](future-work/PRETTY_URL_CACHE_CONTROL_FIX.md)
  * [Prompt Modal Theme Token Gap](future-work/PROMPT_MODAL_THEME_TOKEN_GAP.md)
  * [Recurring Panel Refactor](future-work/RECURRING_PANEL_REFACTOR_PLAN.md)
  * [Recurring Tasks — P3 Follow-ups](future-work/RECURRING_TASKS_P3_FOLLOWUP.md)
  * [Render-Path Unification](future-work/RENDER_PATH_UNIFICATION.md)
  * [Routine Switcher Refactor](future-work/ROUTINE_SWITCHER_REFACTOR_PLAN.md)
  * [Schema 2.6 Migration](future-work/SCHEMA_2_6_PLAN.md)
  * [Task Ordering System](future-work/TASK_ORDERING_SYSTEM_PLAN.md)
  * [Task View Customization](future-work/TASK_VIEW_CUSTOMIZATION_PLAN.md)
  * [Drift Review v2 — Accuracy Correction](future-work/miniCycle-drift-review-v2-accuracy-correction.md)
  * [Drift Review v2](future-work/miniCycle-drift-review-v2.md)


* **Links**
  * [📱 Launch App](https://minicycle.app/miniCycle.html)
  * [🧪 Run Tests](https://minicycle.app/tests/module-test-suite.html)
  * [💾 GitHub Repository](https://github.com/sparkinCreations/miniCycle)

---

* **📦 Archive** _(Completed Work & Historical Docs)_
  * [Archive Overview](archive/README.md)

  * **Modularization (Complete)**
    * [Modularization Guide v4](archive/minicycle_modularization_guide_v4.md)
    * [Modularization Lessons Learned](archive/minicycle_modularization_lessons_learned.md)
    * [Modularization Guide v3](archive/minicycle_modularization_guide_v3.md)
    * [TaskDOM Extraction Plan](archive/TASKDOM_EXTRACTION_PLAN.md)
    * [TaskDOM Split Plan](archive/TASKDOM_SPLIT_PLAN.md)
    * [TaskDOM Completion Summary](archive/TASKDOM_COMPLETION_SUMMARY.md)

  * **Folder Organization (Complete)**
    * [Final Folder Organization](archive/FINAL_FOLDER_ORGANIZATION.md)
    * [Final Module Structure](archive/FINAL-MODULE-STRUCTURE.md)
    * [Folder Structure Refactor Plan](archive/FOLDER_STRUCTURE_REFACTOR_PLAN.md)
    * [Multi-Platform Folder Structure](archive/MULTI_PLATFORM_FOLDER_STRUCTURE.md)
    * [Quick Guide Multi-Platform](archive/QUICK_GUIDE_MULTI_PLATFORM.md)

  * **AppInit Integration (Complete)**
    * [AppInit Integration Plan](archive/APPINIT_INTEGRATION_PLAN.md)
    * [AppInit Implementation Summary](archive/APPINIT_IMPLEMENTATION_SUMMARY.md)
    * [AppInit Rename Summary](archive/APPINIT_RENAME_SUMMARY.md)

  * **Decoupling & Optimization (Complete)**
    * [Decoupling Optimization Plan](archive/DECOUPLING_OPTIMIZATION_PLAN.md)
    * [Decoupling Impact Visualization](archive/DECOUPLING_IMPACT_VISUALIZATION.md)
    * [Coupling Audit Report](archive/COUPLING_AUDIT_REPORT.md)

  * **Feature Implementations (Complete)**
    * [Safari Drag & Drop Fix](archive/SAFARI_DRAGDROP_FIX.md)
    * [Notification Standardization](archive/NOTIFICATION_STANDARDIZATION_PLAN.md)
    * [Menu Settings Extraction](archive/MENU_SETTINGS_EXTRACTION_PLAN.md)
    * [Recurring Integration Guide](archive/recurring-integration-guide.md)

  * **Testing & Undo/Redo (Complete)**
    * [Undo/Redo Phase 1 Complete](archive/PHASE1_COMPLETE_undoRedoManager.md)
    * [Undo/Redo Phase 2 Complete](archive/PHASE2_COMPLETE_undoRedoManager.md)
    * [Undo/Redo Test Plan](archive/TEST_PLAN_undoRedoManager.md)
    * [Automated Testing Integration](archive/AUTOMATED_TESTING_INTEGRATION.md)

  * **Audits & Analysis (Complete)**
    * [Event Listener Audit](archive/EVENT_LISTENER_AUDIT.md)

  * **Historical Updates**
    * [October 2025 Improvements](archive/OCTOBER_2025_IMPROVEMENTS.md)
    * [Recurring Settings Preservation](archive/CHANGELOG-recurring-settings-preservation.md)
