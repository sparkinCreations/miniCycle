...
- Service worker uses inlined `APP_VERSION` (updated by the version script)
...
// appInit is versioned too (all imports use the same APP_VERSION)
...
```javascript
const APP_VERSION = window.APP_VERSION || 'dev-local';
...
2. **Deploy updated files together**
...
12. **appInit singleton consistency** - all imports must use the same version suffix
