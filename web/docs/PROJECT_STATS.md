...
>
> **Machine-readable:** the same numbers ship as JSON at
> <https://minicycle.app/stats.json> (CORS-enabled, regenerated by every build).
...
| `utils/` | 27 | Notifications, device detection, utilities |
...
| `modules/boot/coreBoot.js` | ~773 | Core state & init |
| `modules/boot/featureBoot.js` | ~623 | DI wiring hub |
...
Every count comes from `scripts/collect-stats.cjs` — one implementation shared by
this file, `dist/stats.json`, and the SparkinCreations mirror, so the public
