...
| The migration meets a real document shaped unlike the fixtures | **Dry run on real data (built Sep 2026):** Testing modal -> Diagnostics -> *Dry-Run 2.6 Migration* runs `migrateSchema_2_5_to_2_6` on a clone of the live state and prints the routine key map, task / template / entry counts, the priority-level split and the default level, then checks that the live state is untouched, no 2.5 field name survives anywhere (deep scan), no task is lost or reordered, every task has a complete `autoClear` map, every priority is a level or null, the active routine maps, both version stamps are set, and a second run is a no-op. Never writes. Run it on every device that holds data before the wiring release; the id maker it uses (`GlobalUtils.generateId('routine')`) is the one boot must use too |
| A routine's `taskOptionButtons` map spells `highPriority` / `deleteWhenComplete` — as OPTION IDS (which buttons are visible), not task fields | **Out of scope for 2.6, decided Sep 2026 after the first real-data dry run** (7 routines, 178 tasks: the only "problems" were 8 of these). The ids are shared with `DEFAULT_TASK_OPTION_BUTTONS`, the customizer, the importer allow-list and the labels; renaming them is its own change with no migration value. The dry-run scanner exempts the container |
